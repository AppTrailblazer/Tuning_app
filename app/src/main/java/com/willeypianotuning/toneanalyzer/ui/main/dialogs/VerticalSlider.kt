...
class VerticalSlider : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        iconHigh?.takeIf { progress >= max * 2 / 3 }?.let {
            canvas.drawBitmap(it, null, iconRect, null)
        }
        iconMedium?.takeIf { progress < max * 2 / 3 }?.let {
            canvas.drawBitmap(it, null, iconRect, null)
        }
        iconLow?.takeIf { progress < max / 3 }?.let {
            canvas.drawBitmap(it, null, iconRect, null)
        }
    }
    ...
    fun interface OnSliderProgressChangeListener {
